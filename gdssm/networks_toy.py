...
class Decoder(torch.nn.Module):
    def __init__(self, D_e, D_x=2, mode="full", D_hidden=50):
        super().__init__()
        self.D_hidden = D_hidden
        self.D_e = D_e
        self.D_x = D_x
        ...
        self.log_noise = torch.nn.Parameter(torch.FloatTensor([-20]))
...
class Encoder(torch.nn.Module):
    def __init__(self, D_e, D_x=30, num_modes=1):
        ...
        self.f_msg_1 = torch.nn.Linear(self.D_h, self.D_h, bias=True)
    ...
    def forward(self, x, A):
        ...
        x_enc = torch.tanh(self.f_msg_1(x_emb))
        ...
        P_enc = torch.eye(self.D_e).expand(batch_size, self.num_modes, self.D_e, self.D_e)*1e-6
        P_enc = P_enc.to(device)
              
        weights = self.weight_layer(x_enc).reshape(batch_size, self.num_modes)
